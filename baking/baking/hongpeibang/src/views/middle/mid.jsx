...
class mid extends Component {
    ...
    async componentDidMount(){
        ...
        console.log()
    //    item.image and shareDescription
            ...
            }</div>
            {
                this.state.data.map((v,i)=>(
                    <div className={"MID"} key={i}>
                        ...
                        {
                            this.state.data.map((v)=>{return ((v.item).map(v=>{return (<div className={"MIDdiv"} key={i}> <img alt={"标本"} src={v.image}/> <p className={"MIDP"}>{v.shareDescription}</p> </div>)}))})
                        }
                    </div>
                    ))
            }
